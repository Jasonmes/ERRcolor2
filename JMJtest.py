...
def painting_line(imgCanny, image):
    # 划线
    minLineLength = 0  # 线段以像素为单位的最小长度，根据应用场景设置
    maxLineGap = 5  # 为一条线段的最大允许间隔
    lines = cv2.HoughLinesP(imgCanny,      # 导入轮廓图
                            0.5,            # 线段以像素为单位的距离精度，double类型的，推荐用1.0
                            np.pi / 360,
                            30,           # 值越大，基本上意味着检出的线段越长，检出的线段个数越少
                            minLineLength,
                            maxLineGap)
    print len(lines)
    # 把线画进原图
...
image = cv2.imread('C_test1.PNG')    # load image
...
for (lower, upper) in color1:   # masking using color channel
    ...
    # 侵蚀 膨胀 模糊
    binary = gray
    for i in range(8):         # erosion and dilation loop
        binary = binary
        binary = magic(binary)
...
_ ,canvasB = cv2.threshold(canvasG,0,255,cv2.THRESH_BINARY)
